...
def surface_deviation(reference, candidate, limit):
        ...
        for points in (vertices, np.asarray(vertices)[faces].mean(axis=1)):
            for start in range(0, len(points), 8192):
                ...
                if checked % 65536 < len(batch):
                    report_progress("Collision surface comparison", checked, total,
...
def stabilize_collision_precision(vertices, faces, error_limit):
    ...
    No faces are removed. Every trial restarts from the same double-precision
    candidate and must preserve face orientation in both engine encodings.
...
def simplify_collision(vertices, faces, config, arguments, *, report=None):
    ...
    report.update(method="quadric_edge_collapse", source_triangles=len(faces),
                  max_error_m=config.collision_max_error_m, attempts=[], used_raw_fallback=False,
                  error_scope="All vertices and face centres in both directions, plus complete required capsule sweeps")
    for attempt in range(config.collision_repair_attempts):
        reduction = config.collision_target_reduction/(2**attempt)
        if int(len(faces)*(1-reduction)) < 12:
            continue
        report_progress("Collision reduction", attempt, config.collision_repair_attempts,
                        f"target {reduction:.0%} fewer triangles; error limit {config.collision_max_error_m:g} m")
        with _SIMPLIFIER_LOCK:
            new_vertices, new_faces = fast_simplification.simplify(
                vertices, faces, target_reduction=reduction, agg=5.)
        new_faces = np.asarray(new_faces, np.int64)
        record = dict(target_reduction=reduction, triangles=len(new_faces), accepted=False)
        ...
        try:
            ...
            if record["max_passage_error_m"] > config.collision_max_error_m:
                record["reason"] = "Reduced collider changed measured floor/roof distances"
                continue
            report_progress("Collision surface comparison", detail="checking every vertex and face centre in both directions")
        ...
        report_progress("Collision acceptance", len(report["attempts"]), len(report["attempts"]),
                        f"accepted {len(new_faces):,} triangles ({report['achieved_reduction']:.0%} reduction)")
        return new_vertices, new_faces
    fallback, precision = stabilize_collision_precision(vertices, faces, config.collision_max_error_m)
    if not precision["passed"]:
    ...
    if passage_deviation(reference, fallback_inspection) > config.collision_max_error_m:
    ...
    deviation = surface_deviation((vertices, faces), (fallback, faces), config.collision_max_error_m)
    if not deviation["passed"]:
