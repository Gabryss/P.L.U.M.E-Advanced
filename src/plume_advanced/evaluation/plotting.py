...
def _scalability_figure(source: Path, output: Path) -> list[Path]:
    ...
    for mode, marker in (("dense", "o"), ("tiled", "s")):
        selected = [row for row in rows if row.get("storage_mode_requested") == mode]
        lengths = [float(row["route_length_requested_m"]) / 1000.0 for row in selected]
        axes[0].scatter(
            lengths,
            [float(row["wall_time_s"]) for row in selected],
            label=mode,
            marker=marker,
        )
        axes[1].scatter(
            lengths,
            [float(row["peak_rss_gib"]) for row in selected],
            label=mode,
            marker=marker,
        )
    axes[0].set(xlabel="Requested route length (km)", ylabel="Wall time (s)")
    axes[1].set(xlabel="Requested route length (km)", ylabel="Peak RSS (GiB)")
    for axis in axes:
        axis.grid(alpha=0.25)
    axes[0].legend()
