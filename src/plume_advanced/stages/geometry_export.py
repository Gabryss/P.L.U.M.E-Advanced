...
def _cave_primitive_payload(
    *,
    ...
    if density_grid is None:
        ...
        reference_normals = trimesh.Trimesh(vertices=canonical_vertices, faces=face_indices,
                                            process=False).vertex_normals
        canonical_normals = density_surface_normals(
            density_grid, canonical_vertices, reference_normals, filter_voxels=normal_filter_voxels,
        )
...
def density_surface_normals(
    grid: VoxelGrid | TiledVoxelGrid,
    vertices: np.ndarray,
    reference_normals: np.ndarray,
    *,
    filter_voxels: float = 1.2,
) -> np.ndarray:
    """Shade the implicit surface without triangle/grid-aligned normal bands.

    This changes normals only. A modest reconstruction filter on the distance
    field makes normals independent of the marching-cubes triangle pattern.
    Tiled gradients use neighboring scalar samples, including at tile seams.
    """
    normals = np.asarray(reference_normals, dtype=float).copy()
    if len(vertices) == 0:
        return normals
    coords = (np.asarray(vertices) - np.asarray(grid.origin)) / grid.voxel_size

    def sample(density: np.ndarray, local_coords: np.ndarray) -> np.ndarray:
        smooth = ndimage.gaussian_filter(density, sigma=filter_voxels, truncate=3.0, mode="nearest")
        gradient = [np.gradient(smooth, axis=axis) if smooth.shape[axis] > 1 else np.zeros_like(smooth)
                    for axis in range(3)]
        return -np.column_stack([
            ndimage.map_coordinates(component, local_coords.T, order=1, mode="nearest", prefilter=False)
            for component in gradient
        ])

    if isinstance(grid, VoxelGrid):
        values = sample(grid.density, coords)
    else:
        values = np.zeros_like(normals)
        keys, inverse = np.unique(np.floor(coords/grid.tile_size).astype(int), axis=0, return_inverse=True)
        ordered = np.argsort(inverse, kind="stable")
        offsets = np.concatenate(([0], np.cumsum(np.bincount(inverse))))
        halo = math.ceil(3.*filter_voxels)+2
        for i, key_array in enumerate(keys):
            key = (int(key_array[0]), int(key_array[1]), int(key_array[2]))
            tile = grid.tiles.get(key)
            if tile is None:
                continue  # retain mesh normals outside the sampled volume
            start = key_array*grid.tile_size - halo
            shape = np.asarray(tile.shape) + 2*halo
            density = np.full(tuple(shape), grid.iso_level-8., dtype=np.float32)
            reach = math.ceil(halo/grid.tile_size)
            for delta in np.ndindex(*((2*reach+1,)*3)):
                other_key = (key[0]+delta[0]-reach, key[1]+delta[1]-reach, key[2]+delta[2]-reach)
                other = grid.tiles.get(other_key)
                if other is None:
                    continue
                other_start = np.asarray(other_key)*grid.tile_size
                low = np.maximum(start, other_start)
                high = np.minimum(start+shape, other_start+other.shape)
                if np.any(high <= low):
                    continue
                dest = tuple(slice(int(a), int(b)) for a, b in zip(low-start, high-start))
                src = tuple(slice(int(a), int(b)) for a, b in zip(low-other_start, high-other_start))
                density[dest] = other[src]
            selected = ordered[offsets[i]:offsets[i+1]]
            values[selected] = sample(density, coords[selected]-start)
    lengths = np.linalg.norm(values, axis=1)
    usable = lengths > 1e-8
    usable &= np.all((coords >= 0.) & (coords <= np.asarray(grid.shape)-1), axis=1)
    values[usable] /= lengths[usable, None]
    # Exporters can orient surfaces either inward or outward. Preserve that
    # orientation, including closed internal obstacle shells.
    values[np.sum(values*normals, axis=1) < 0.] *= -1.
    normals[usable] = values[usable]
    return normals
