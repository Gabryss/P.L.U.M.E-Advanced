...
def _network(density: float = 1.0) -> CaveNetwork:
    ...
    return CaveNetwork(
        CaveNetworkConfig(network_density=density),
        nodes,
        (segment,),
        (),
        np.zeros((2, 2), bool),
...
def test_dashboard_payload_is_deterministic_and_density_fields_ordered() -> None:
    base = {
        ...
        "segment_count": 6,
        "edge_count": 6,
        "main_route_length_m": 100.0,
        "cyclomatic_number": 2,
        "stacked_segment_count": 2,
        "vertical_capture_count": 1,
        "split_merge_region_count": 2,
        "connected_component_count": 1,
        "zero_flux_segment_count": 0,
        "normalized_topology": {"branch_segment_fraction": 0.5},
    }
    density = {
        3.0: [base, {**base, "cyclomatic_number": 4}],
        0.0: {
            **base,
            "segment_count": 1,
            "edge_count": 1,
            "cyclomatic_number": 0,
            "stacked_segment_count": 0,
            "vertical_capture_count": 0,
            "split_merge_region_count": 0,
            "normalized_topology": {"branch_segment_fraction": 0.0},
        },
    }
    first = dashboard_payload(_network(), density_sweep=density)
    second = dashboard_payload(
        _network(), density_sweep={key: density[key] for key in reversed(list(density))}
    )
    assert json.dumps(first, sort_keys=True) == json.dumps(second, sort_keys=True)
    assert [row["density"] for row in first["density_sweep"]] == [0.0, 3.0]
    assert (
        first["density_sweep"][1]["cyclomatic_per_km_p90"]
        > first["density_sweep"][1]["cyclomatic_per_km_p10"]
    )
...
def test_dashboard_png_and_json_sidecar_are_valid(tmp_path: Path) -> None:
    output, sidecar = render_diagnostic_dashboard(
        _network(), output_path=tmp_path / "dashboard.png", provenance={"seed": 7, "world": "earth"}
    )
        ...
        "records": [
            {
                "aspect_ratio": 1.0,
                "compactness": 0.6,
                "floor_residual_norm": 0.1,
                "roof_asymmetry_norm": 0.2,
            },
            {
                "aspect_ratio": 1.5,
                "compactness": 0.8,
                "floor_residual_norm": 0.2,
                "roof_asymmetry_norm": 0.3,
            },
            {
                "aspect_ratio": 2.0,
                "compactness": 0.9,
                "floor_residual_norm": 0.3,
                "roof_asymmetry_norm": 0.4,
            },
        ],
    }
