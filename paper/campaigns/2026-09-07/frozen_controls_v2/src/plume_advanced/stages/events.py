...
class GeologicalEventGenerator:
    ...
    def _roof_demand_ratio(self, sample: SectionSample) -> float:
        ...
        return RoofStabilityModel(
            self.config.gravity_m_s2, self.config.rock_density_kg_m3,
            self.config.effective_tensile_strength_pa, self.config.roof_safety_factor,
        ).assess(
            width_m=sample.tube_width, height_m=sample.tube_height,
            floor_depth_m=sample.tube_height + sample.roof_thickness,
        ).demand_ratio
