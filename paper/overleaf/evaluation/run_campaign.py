...
def main():
    ...
    for name in COMMANDS:
        ...
        if name == "controllability":
            command_env["PYTHONPATH"] = str(CAMPAIGN / "frozen_controls_v2/src")
            command_env["PLUME_EVALUATION_WORKERS"] = "6"
        if name == "morphometry":
        ...
        status["commands"][name] = {
            ...
            "package_source": command_env["PYTHONPATH"],
            "configured_network_workers": int(command_env["PLUME_EVALUATION_WORKERS"]),
        }
        save()
        print(f"Starting {name}", flush=True)
        with (log_root / f"{name}.log").open("a") as log:
            result = subprocess.run(command, cwd=ROOT, env=command_env, stdout=log,
