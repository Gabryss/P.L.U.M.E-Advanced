...
def test_calibration_filter_never_reads_reserved_coordinates(tmp_path: Path, monkeypatch) -> None:
    for name in ("Calibration", "Reserved"):
        cave = tmp_path / name
        cave.mkdir()
        (cave / "section_1.txt").write_text("0 0\n2 0\n2 1\n0 1\n0 0\n")
    original = Path.read_text
    seen = []

    def checked_read(path, *args, **kwargs):
        assert path.parent.name != "Reserved", "Held-out coordinates were opened"
        seen.append(path)
        return original(path, *args, **kwargs)

    monkeypatch.setattr(Path, "read_text", checked_read)
    sections, rejections = load_pdc(tmp_path, cave_ids={"Calibration"})
    assert len(sections) == 1 and sections[0].reference_cave_id == "Calibration"
