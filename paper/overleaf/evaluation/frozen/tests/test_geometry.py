...
class GeometryTests(unittest.TestCase):
    def test_geometry_stage_stamps_voxels_and_builds_isosurface_mesh(self) -> None:
        ...
        # Section morphology can now form branches smaller than the Stage-B
        # routing radius; resolution diagnostics must reflect the real input.
        minimum_diameter_samples = min(
            sample.tube_width
            for field in section_field.segment_fields for sample in field.samples
        ) / project_config.geometry.voxel_size
        self.assertAlmostEqual(
